...
async function getPendingOffers(req: Request, res: Response) {
    try {
        ...
        const offersWithSkills = await Promise.all(offers.map(async (offer) => {
        ...
        const offersWithUserData = await Promise.all(offersWithSkills.map(async (offer) => {
...
async function acceptOffer(req: Request, res: Response) {
    try {
        ...
        if (!targetUserSkill) {
            return res.status(400).json({ message: 'User does not have the required skill' });
        }
